   ...
   // if the game has ended
    ...
    public void processButtonClick(int col) {
        if (gameEnd) {
            ...
            // make sure the column is not ful
            if (curGame.checkIfFree(col)) {
                // find the row to insert at for the screen
                int row = -1;
                ...
                if (curGame.checkForWin(col)) {
                    screen.setMessage("Player " + players[currentPlayer] + " Won!");
                    currentPlayer = 0;
                    gameEnd = true;
                    ...
                    // updates the currentPlayer if neither a win or tie
