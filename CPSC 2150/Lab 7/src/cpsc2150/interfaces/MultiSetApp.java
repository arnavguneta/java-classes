...
public class MultiSetApp {
	public static void main(String[] args) {
		...
		ISet<Integer> set1 = new ArraySet<>();
		ISet<Integer> set2 = new ArraySet<>();
	 ...
	 */
	static private void setInit(ISet<Integer> set, Scanner scanner) {
		String c = "";
		int insertSet;
		do {
			...
			if (set.getSize() < ISet.MAX_SIZE) {
				...
				c = scanner.next();
				// if it is the exit case, return
				if (c.equalsIgnoreCase("q")) {
					return;
				}
				// parse the integer and add it to the set
				insertSet = Integer.parseInt(c);
